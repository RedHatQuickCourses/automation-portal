...
. Login to the Openshift Cluster using {openshift_cluster_console_url} with Username: {openshift_cluster_admin_username} and Password {openshift_cluster_admin_password} and go to Network -> Routes to access the Ansible Automation Platform. 

. Select the project as *aap* and open your Ansible Automation Platform instance in a web browser. 
+
image::5.1.png[]

. Go to `Workload -> Secrets -> aap-admin-password` and use the `administrator` credentials provided to you.
+
image::5.2.png[]
+
image::5.3.png[]

Open your Ansible Automation Platform instance in a web browser using {aap_controller_web_url} and log is as user {controller_username} with the password you retrieved from the {controller_password} secret in the previous step.
...
* *Redirect URIs:* Enter a placeholder URL for now: `https://<automation-portal>/api/auth/rhaap/handler/frame` (we'll update this later with the actual portal deployment URL)
+
image::5.4.png[]
...
. Go to the API 'https://{aap_controller_web_url}/api/gateway/v1/tokens/' and replace the {aap_controller_web_url} with the actual hostname of the Ansible Automation Platform. 
+
image::5.6.png[]

.  Configure the Token according to the screen shot and POST the Request.
+
...
----
oc login -u admin -p {openshift_cluster_admin_password} {openshift_api_server_url}
----
+ 
image::5.8.png[]
