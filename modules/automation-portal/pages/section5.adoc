...
. Login to the Openshift Cluster and go to Network -> Routes to access the Ansible Automation Platform. 
...
[source,bash]
----
# Example AAP URL (replace with your instance from OpenShift Route)
...
. Go to the API `https://<aap-hostname>/api/gateway/v1/tokens/` and replace the <aap-hostname> with the actual hostname of the Ansible Automation Platform. 
+
image::5.6.png[]
